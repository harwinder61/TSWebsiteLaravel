...
class ScheduledInactive extends Command
{
    ...
    //         Log::info('User ID: ' . $user->id . ' | User Email: ' . $user->email);
    ...
    //                 ->where('created_at', '>=', Carbon::now()->subDays(14)) // Ensure it's within the 14-day threshold
    //                 ->first();
    ...
    //                 SmsLogs::create(['message' => $message, 'to' => $phone, 'status' => 1, 'user_id' => $user->id, 'from' => env('TWILIO_PHONE_NUMBER'), 'message_sent' => true]);
    //             } catch (\Exception $e) {
    //                 Log::error('Failed to send SMS for User ID: ' . $user->id . ' - Error: ' . $e->getMessage());
    //                 SmsLogs::create(['message' => $message, 'to' => $phone, 'status' => 0, 'user_id' => $user->id, 'from' => env('TWILIO_PHONE_NUMBER'), 'message_sent' => false]);
    //             }
    //         } else {
    //             Log::info('Skipping SMS for User ID: ' . $user->id . ' - No valid phone number');
    //         }
    ...
    //         SmsLogs::create(['message' => 'Invalid phone number format', 'to' => $phone, 'status' => 0, 'user_id' => $user->id, 'from' => env('TWILIO_PHONE_NUMBER')]);
    ...
    //         SmsLogs::create(['message' => $message, 'to' => $phone, 'status' => 1, 'user_id' => $user->id, 'from' => env('TWILIO_PHONE_NUMBER')]);
    //         return "SMS sent successfully!";
    //     } catch (\Exception $e) {
    //         SmsLogs::create(['message' => $message, 'to' => $phone, 'status' => 0, 'user_id' => $user->id, 'from' => env('TWILIO_PHONE_NUMBER')]);
