...
class ScheduledSms extends Command
{
        ...
        foreach ($expiringAdverts as $advert) {
            ...
            if ($advert->user && $advert->user->profile && $advert->user->profile->phone_number) {
                try {
                    ...
                    $smsLog = SmsLogs::where('user_id', $advert->user->id)
                        ->where('status', 1)
                        ->where('to', $phone)
                        ->where('message', 'like', '%advert_ending_in_48_hours%')
                        ->first();
    
                    if ($smsLog) {
                        Log::info('SMS already sent for Subscription ID: ' . $advert->id . ' - Skipping SMS.');
                        continue; // Skip sending SMS
                    }
                    ...
                    Log::error('Failed to send SMS for User ID: ' . $advert->user->id . ' - Error: ' . $e->getMessage());
                    SmsLogs::create([
                        'message' => $message, 
                        'to' => $phone, 
                        'status' => 0, 
    ...
    {
        Log::info('>>>>>>>>>>>>><<<<<<<<<<advert_ending_in_48_hours message >>>>>>>>>>><<<<<<<<<<<<<<<<>>>><<<<<<<<');
        ...
        try {
            ...
            return [
                ...
                'message_id' => $twilioResponse->sid // Include Twilio message ID
            ];
        } catch (\Exception $e) {
            // Log failed SMS
            SmsLogs::create([
